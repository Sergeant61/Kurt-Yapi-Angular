...
export class TirKamyonService {
  ...
  getAll(): Observable<ApiResponse<TirKamyon[]>> {
    return this.http.get<ApiResponse<TirKamyon[]>>(this.path + this.baseUrl, { headers: this.authService.getHeaders() })
    .pipe(map(res => {
      if (!res.success) {
        if (this.errorService.getErrorParse(res)) {
          return res;
  ...
  get(id: string): Observable<ApiResponse<TirKamyon>> {
    return this.http.get<ApiResponse<TirKamyon>>(this.path + this.baseUrl + '/' + id, { headers: this.authService.getHeaders() })
    .pipe(map(res => {
      if (!res.success) {
        if (this.errorService.getErrorParse(res)) {
          return res;
  ...
  put(id: string, data: TirKamyon): Observable<ApiResponse<TirKamyon>> {
    return this.http.put<ApiResponse<TirKamyon>>(this.path + this.baseUrl + '/' + id, data, { headers: this.authService.getHeaders() })
    .pipe(map(res => {
      if (!res.success) {
        if (this.errorService.getErrorParse(res)) {
          return res;
  ...
  delete(id: string): Observable<ApiResponse<TirKamyon>> {
    return this.http.delete<ApiResponse<TirKamyon>>(this.path + this.baseUrl + '/' + id, { headers: this.authService.getHeaders() })
    .pipe(map(res => {
      if (!res.success) {
        if (this.errorService.getErrorParse(res)) {
          return res;
